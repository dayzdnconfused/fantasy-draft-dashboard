    ...
    if st.sidebar.button("🔄 Sync Live ESPN Rosters"):
        with st.spinner("Connecting to ESPN Servers..."):
            try:
                ...
                # Using 2026 based on your successful test
...
def ui_pitcher_list_scanner():
    ...
    # We add a quick input for your Team ID, defaulting to 3
    user_team_id = st.number_input("Your ESPN Team ID", min_value=1, max_value=20, value=3)
        ...
        with st.spinner("Scraping Pitcher List and cross-referencing rosters..."):
            ...
            if not pl_df.empty:
                ...
                with col2:
                    ...
                    st.dataframe(
                        my_pl_df[['PL_Rank', 'Player_Name']].sort_values('PL_Rank', ascending=False), # Sorted worst to best so drop candidates are on top
                        hide_index=True,
                        use_container_width=True
                    )
            else:
